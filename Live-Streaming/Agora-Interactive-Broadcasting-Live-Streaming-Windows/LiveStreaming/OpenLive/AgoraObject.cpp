...
int CAgoraObject::RemovePublishStreamUrl(const char *url)
{
	return m_lpAgoraEngine->removePublishStreamUrl(url);
}

int CAgoraObject::AddPublishStreamUrl(const char *url, bool transcodingEnabled)
{
	return  m_lpAgoraEngine->addPublishStreamUrl(url, transcodingEnabled);
}

int CAgoraObject::SetLiveTranscoding(const LiveTranscoding &transcoding)
{
	return m_lpAgoraEngine->setLiveTranscoding(transcoding);
}
...
BOOL CAgoraObject::KeepPreRotation(bool bRotate)
{
	...
	if (bRotate){
		...
		nRet = apm->setParameters("{\"che.video.keep_prerotation\":false}");
	}
...
BOOL CAgoraObject::SetWaterMark(LPCTSTR lpFilePath, LPCRECT lpRect)
{
	RtcImage vwm;
	...
	::WideCharToMultiByte(CP_ACP, 0, lpFilePath, -1, szURL, MAX_PATH, NULL, NULL);
	vwm.url = szURL;
#else
	vwm.url = lpURL;
#endif
	...
	vwm.x = lpRect->left;
	vwm.y = lpRect->top;
	vwm.width = lpRect->right - lpRect->left;
	vwm.height = lpRect->bottom - lpRect->top;

	int nRet = m_lpAgoraEngine->addVideoWatermark(vwm);

	return nRet == 0 ? TRUE : FALSE;
}

void CAgoraObject::RemoveWaterMark()
{
	m_lpAgoraEngine->clearVideoWatermarks();
}
