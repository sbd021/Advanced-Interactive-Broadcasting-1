...
void CAgoraObject::SetInjectedUrl(LPCTSTR lpInjectedUrl)
{
	m_strInjectUrl = lpInjectedUrl;
}

CString CAgoraObject::GetInjectedUrl()
{
	return m_strInjectUrl;
}
...
BOOL CAgoraObject::StreamInjected(LPCTSTR lpInjectedURL)
{
	...
	int nRet = 0;
	if (m_lpAgoraEngine) {
		InjectStreamConfig isc;
		nRet = m_lpAgoraEngine->addInjectStreamUrl(CStringA(CString(lpInjectedURL).GetBuffer(0)).GetBuffer(0),isc);
